...
function transformToLis(obj){
  // Solution code here...
  return Object.entries(obj).map(keyValues => `<li>${keyValues.join(': ')}</li>`);
}
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((arrOfNums, count) => arrOfNums = count++);
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((starWarsDataArray, charName) => {
    starWarsDataArray.push(charName.name);
    return starWarsDataArray;
  }, []);
};
